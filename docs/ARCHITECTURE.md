...
`app/services/ai/` implementa un gateway con proveedores intercambiables (hoy: Ollama local). Política `local_only` por defecto — nunca sale a un proveedor externo salvo cambio explícito de configuración. El modelo (`qwen2.5:3b` por defecto) recibe asunto, nombre y dirección real del remitente, fecha y una vista previa acotada de cada mensaje del expediente — nunca el cuerpo completo ni adjuntos. **Los participantes no se enmascaran** (una versión anterior de este documento decía lo contrario; ver [`STATUS.md`](STATUS.md)). Devuelve un resumen validado contra un esquema fijo, incluida una conclusión sugerida que el auditor siempre confirma a mano.
