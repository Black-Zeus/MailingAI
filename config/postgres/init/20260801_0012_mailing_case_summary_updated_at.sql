-- =============================================================================
-- Mailing AI - Expone mailing.cases.updated_at en mailing.v_case_summary.
--
-- Lo necesita el frontend para bloqueo optimista al guardar cambios de un
-- expediente (conclusion, seguimiento, resumen de IA): manda de vuelta el
-- updated_at que vio al cargar, y el backend solo aplica el UPDATE si nadie
-- lo cambio mientras tanto (ver cases_repository.update_case).
--
...
GROUP BY c.case_id, c.case_type, c.external_code, c.title, c.status, c.confidence, c.outcome,
         ...
         c.previous_owner_label, c.updated_at;
